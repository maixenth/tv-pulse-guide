...
SELECT cron.unschedule('sync-channels-hourly') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'sync-channels-hourly'
);
...
DROP POLICY IF EXISTS "Anyone can view sync history" ON public.channel_sync_history;
